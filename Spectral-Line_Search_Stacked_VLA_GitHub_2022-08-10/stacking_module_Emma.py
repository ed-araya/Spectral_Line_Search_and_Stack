...
def chans_rm_continuum(cube_input):
    ...
    for cube in cube_input:
        ...
        #unclear why this is done, seems not needed here.
        ...
        for i in range(num):
            ...
            if len(chan_stats['rms'])==0:
                ...
                if chan_stats['flux'][0]>lower_lim and chan_stats['flux'][0]<upper_lim:
                    #EA Question: I would expect a channel
                    #with a flux between the lower and
                    #upper limit to be included in the
                    #continuum substraction. I guess
                    #we are selecting here the outlier
                    #channels, not the channels for
                    #continuum substraction??
                    # It seems to me as a mistake, so
                    # I commented it out and changed the
                    # if statement
                #if chan_stats['flux'][0]<lower_lim or chan_stats['flux'][0]>upper_lim:
                    pass
                else:
                    channs_end=i-1
                    if chans<channs_end:
